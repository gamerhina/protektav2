...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
        ...
        @if($selectedPA)
            <div class="mb-6 flex items-center justify-between p-4 bg-blue-50 border border-blue-200 rounded-2xl shadow-sm">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-xl bg-blue-600 text-white flex items-center justify-center">
                        <i class="fas fa-user-check"></i>
                    </div>
                    <div>
                        <p class="text-[10px] uppercase tracking-widest font-bold text-blue-400">Filter Pembimbing Akademik</p>
                        <h2 class="text-sm font-bold text-blue-900">{{ $selectedPA->nama }} <span class="text-blue-500 font-normal">(NIP. {{ $selectedPA->nip }})</span></h2>
                    </div>
                </div>
                <a href="{{ route('admin.mahasiswa.index') }}" class="text-xs font-bold text-blue-600 hover:text-blue-800 bg-white px-4 py-2 rounded-xl shadow-sm border border-blue-100 transition-all">
                    Hapus Filter
                </a>
            </div>
        @endif
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead>
                    <tr>
                        ...
                        <x-sortable-th column="npm" label="NPM" :default-sort="$defaultSort" :default-direction="$defaultDirection" />
                        <x-sortable-th column="email" label="Email" :default-sort="$defaultSort" :default-direction="$defaultDirection" />
                        <th class="px-6 py-3 text-left text-xs font-semibold tracking-[0.2em] text-gray-500 uppercase bg-gray-50">Pembimbing Akad.</th>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors">
                        <td class="px-6 py-4 text-sm font-semibold text-gray-800">{{ $mahasiswa->nama }}</td>
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $mahasiswa->npm }}</td>
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $mahasiswa->email }}</td>
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $mahasiswa->pembimbingAkademik->nama ?? '-' }}</td>
