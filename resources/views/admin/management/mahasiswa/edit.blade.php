...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
        <h1 class="text-2xl font-semibold text-gray-800 mb-6">Ubah Mahasiswa</h1>
        ...
        <form action="{{ route('admin.mahasiswa.update', $mahasiswa->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-6">
                ...
                <div>
                    <label for="pembimbing_akademik_id" class="block text-sm font-medium text-gray-700 mb-1">Pembimbing Akademik (PA)</label>
                    <select
                        name="pembimbing_akademik_id"
                        id="pembimbing_akademik_id"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md @error('pembimbing_akademik_id') border-red-500 @enderror shadow-sm focus:border-blue-500 focus:ring-blue-500"
                    >
                        <option value="">-- Pilih Pembimbing Akademik --</option>
                        @foreach($dosens as $d)
                            <option value="{{ $d->id }}" {{ old('pembimbing_akademik_id', $mahasiswa->pembimbing_akademik_id) == $d->id ? 'selected' : '' }}>
                                {{ $d->nama }} (NIP: {{ $d->nip }})
                            </option>
                        @endforeach
                    </select>
                    @error('pembimbing_akademik_id')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="bg-gray-50 p-5 rounded-2xl border border-gray-200 group hover:border-blue-200 transition-all">
                    <div class="flex items-start justify-between mb-4">
                        ...
                        <span class="flex-shrink-0 bg-{{ $mahasiswa->foto ? 'emerald' : 'gray' }}-100 text-{{ $mahasiswa->foto ? 'emerald' : 'gray' }}-700 text-[10px] font-bold px-2 py-1 rounded-full">
                            {{ $mahasiswa->foto ? 'SUDAH ADA' : 'BELUM ADA' }}
                        </span>
                    </div>
                    ...
                    <div class="flex items-start gap-4">
                        <div class="flex-shrink-0">
                            @if($mahasiswa->foto)
                                <img src="{{ asset('uploads/' . $mahasiswa->foto) }}" alt="Current Photo" class="w-16 h-16 rounded-xl object-cover border border-gray-100 shadow-sm">
                            @else
