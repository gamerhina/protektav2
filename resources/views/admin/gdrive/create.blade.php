...
@section('title', 'Tambah GDrive Folder')
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
            <div>
                <h1 class="text-2xl font-semibold text-gray-800">Tambah Google Drive Folder Baru</h1>
                <p class="text-sm text-gray-600">Silakan masukkan informasi folder Google Drive</p>
            </div>
        </div>

        <form action="{{ route('admin.gdrive.store') }}" method="POST">
            @csrf
