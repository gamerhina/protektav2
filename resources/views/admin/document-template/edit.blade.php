...
@section('title', 'Edit Template Dokumen: ' . $template->nama)
...
<div class="max-w-[98%] mx-auto px-2 py-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Edit Template: {{ $template->nama }}</h1>
            <p class="text-slate-500 mt-1 flex items-center gap-2">
                <i class="fas fa-file-invoice text-indigo-500"></i> Seminar Document Designer for <strong>{{ $seminarJenis->nama }}</strong>
            </p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.document-template.index', $seminarJenis) }}" class="px-5 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-600 font-semibold hover:bg-slate-50 transition-all flex items-center gap-2">
            ...
            <button type="button" id="btn-save-master" class="px-6 py-2.5 rounded-xl bg-indigo-600 text-white font-bold shadow-lg shadow-indigo-100 hover:bg-indigo-700 hover:-translate-y-0.5 transition-all flex items-center gap-2">
                <i class="fas fa-check"></i> Perbarui Dokumen
            </button>
        </div>
    </div>

    <form id="master-form" method="POST" action="{{ route('admin.document-template.update', [$seminarJenis, $template]) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-8">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    <div class="form-group lg:col-span-1">
                        ...
                        <input name="nama" value="{{ old('nama', $template->nama) }}" class="w-full px-4 py-2.5 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 transition-all text-sm" required>
                    </div>

                    <div class="form-group lg:col-span-1">
                        <label class="block text-sm font-bold text-slate-700 mb-3 flex items-center gap-2">
                            <i class="fas fa-fingerprint text-indigo-500"></i> Kode Unique
                        </label>
                        <input name="kode" value="{{ old('kode', $template->kode) }}" class="w-full px-4 py-2.5 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 transition-all font-mono text-xs bg-slate-50 cursor-not-allowed" readonly>
                    </div>

                    <div class="form-group lg:col-span-1">
                        <label class="block text-sm font-bold text-slate-700 mb-3 flex items-center gap-2">
                            <i class="fas fa-image text-indigo-500"></i> Kop Surat
                        </label>
                        
                        @if($template->header_image_path)
                            <div class="mb-4 relative group rounded-2xl overflow-hidden border border-slate-100 bg-slate-50 p-2">
                                <img src="{{ Storage::url($template->header_image_path) }}" class="w-full h-auto rounded-xl shadow-sm max-h-[60px] object-contain" alt="Current Header">
                                <div class="mt-2 flex items-center justify-between">
                                    <label class="flex items-center gap-2 text-[10px] text-red-500 font-bold cursor-pointer hover:text-red-600 transition-all">
                                        <input type="checkbox" name="remove_header_image" value="1" class="rounded border-red-300 text-red-500 focus:ring-red-500 w-3 h-3">
                                        Hapus Kop
                                    </label>
                                </div>
                            </div>
                    ...
                    <div class="form-group lg:col-span-1">
                        ...
                        <div class="flex items-center gap-4 mt-2">
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="header_repeat" value="1" {{ old('header_repeat', $template->header_repeat) ? 'checked' : '' }} class="sr-only peer">
                                <div class="w-11 h-6 bg-slate-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-indigo-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-indigo-600"></div>
                                <span class="ml-3 text-sm font-medium text-slate-600">Ya, Ulangi</span>
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="form-group lg:col-span-1">
                        ...
                        <select name="header_visibility" id="header_visibility" class="w-full px-4 py-2.5 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 transition-all text-sm">
                            <option value="all" {{ old('header_visibility', $template->header_visibility) == 'all' ? 'selected' : '' }}>Semua Halaman</option>
                            <option value="first_only" {{ old('header_visibility', $template->header_visibility) == 'first_only' ? 'selected' : '' }}>Hanya Halaman 1</option>
                            <option value="except_first" {{ old('header_visibility', $template->header_visibility) == 'except_first' ? 'selected' : '' }}>Semua kecuali Hal. 1</option>
                            <option value="custom" {{ old('header_visibility', $template->header_visibility) == 'custom' ? 'selected' : '' }}>Halaman Kustom (1,3,4)</option>
                        </select>
                    </div>

                    <div id="custom_pages_container" class="form-group lg:col-span-1 {{ old('header_visibility', $template->header_visibility) == 'custom' ? '' : 'hidden' }}">
                        <label class="block text-sm font-bold text-slate-700 mb-3 flex items-center gap-2">
                            <i class="fas fa-list-ol text-indigo-500"></i> No. Halaman
                        </label>
                        <input name="header_custom_pages" value="{{ old('header_custom_pages', $template->header_custom_pages) }}" class="w-full px-4 py-2.5 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 transition-all text-sm" placeholder="Misal: 1,3,4">
                    </div>
                    ...
                    <div class="form-group lg:col-span-1">
                        ...
                        <select name="signature_method" class="w-full px-4 py-2.5 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 transition-all text-sm">
                            <option value="qr_code" {{ old('signature_method', $template->signature_method) == 'qr_code' ? 'selected' : '' }}>QR Code (Otomatis)</option>
                            <option value="manual" {{ old('signature_method', $template->signature_method) == 'manual' ? 'selected' : '' }}>Manual (Canvas)</option>
                        </select>
                    </div>

                    <div class="col-span-full border-t border-slate-100 mt-4 pt-6">
                                ...
                                <div class="flex flex-col gap-3">
                                    <label class="flex items-center gap-2 cursor-pointer group">
                                        <input type="checkbox" name="download_rules[diajukan][]" value="mahasiswa" 
                                            {{ in_array('mahasiswa', $template->download_rules['diajukan'] ?? []) ? 'checked' : '' }}
                                    ...
                                    <label class="flex items-center gap-2 cursor-pointer group">
                                        <input type="checkbox" name="download_rules[diajukan][]" value="dosen" 
                                            {{ in_array('dosen', $template->download_rules['diajukan'] ?? []) ? 'checked' : '' }}
                                            class="w-5 h-5 rounded border-slate-300 text-yellow-600 focus:ring-yellow-500">
                                ...
                                <div class="flex flex-col gap-3">
                                    <label class="flex items-center gap-2 cursor-pointer group">
                                        <input type="checkbox" name="download_rules[disetujui][]" value="mahasiswa" 
                                            {{ in_array('mahasiswa', $template->download_rules['disetujui'] ?? []) ? 'checked' : '' }}
                                            class="w-5 h-5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
                                        <span class="text-sm text-slate-600 group-hover:text-slate-900 transition-colors">Mahasiswa</span>
                                    </label>
                                    <label class="flex items-center gap-2 cursor-pointer group">
                                        <input type="checkbox" name="download_rules[disetujui][]" value="dosen" 
                                            {{ in_array('dosen', $template->download_rules['disetujui'] ?? []) ? 'checked' : '' }}
                                            class="w-5 h-5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
                                        <span class="text-sm text-slate-600 group-hover:text-slate-900 transition-colors">Dosen</span>
                                    </label>
                                </div>
                                ...
                                <div class="flex flex-col gap-3">
                                    <label class="flex items-center gap-2 cursor-pointer group">
                                        <input type="checkbox" name="download_rules[belum_lengkap][]" value="mahasiswa" 
                                            {{ in_array('mahasiswa', $template->download_rules['belum_lengkap'] ?? []) ? 'checked' : '' }}
                                    ...
                                    <label class="flex items-center gap-2 cursor-pointer group">
                                        <input type="checkbox" name="download_rules[belum_lengkap][]" value="dosen" 
                                            {{ in_array('dosen', $template->download_rules['belum_lengkap'] ?? []) ? 'checked' : '' }}
                                            class="w-5 h-5 rounded border-slate-300 text-orange-600 focus:ring-orange-500">
                                        <span class="text-sm text-slate-600 group-hover:text-slate-900 transition-colors">Dosen</span>
                                    </label>
                                </div>
                                ...
                                <div class="flex flex-col gap-3">
                                    <label class="flex items-center gap-2 cursor-pointer group">
                                        <input type="checkbox" name="download_rules[selesai][]" value="mahasiswa" 
                                            {{ in_array('mahasiswa', $template->download_rules['selesai'] ?? []) ? 'checked' : '' }}
                                    ...
                                    <label class="flex items-center gap-2 cursor-pointer group">
                                        <input type="checkbox" name="download_rules[selesai][]" value="dosen" 
                                            {{ in_array('dosen', $template->download_rules['selesai'] ?? []) ? 'checked' : '' }}
                                            class="w-5 h-5 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500">
                                        <span class="text-sm text-slate-600 group-hover:text-slate-900 transition-colors">Dosen</span>
                                    </label>
                                </div>
            ...
            <div class="bg-white rounded-3xl shadow-xl shadow-slate-100 border border-slate-100 overflow-hidden">
                <div class="p-5 bg-slate-50/80 border-b border-slate-100 flex items-center justify-between">
                    <span class="text-xs font-bold text-slate-500 uppercase flex items-center gap-2">
                        <i class="fas fa-magic text-indigo-400"></i> Document Designer
                        <span id="editor-loading" class="text-[10px] text-blue-600 font-normal hidden">
                            <i class="fas fa-spinner fa-spin"></i> Loading editor...
                        </span>
                        <span id="editor-ready" class="text-[10px] text-green-600 font-normal hidden">
                ...
                <div class="p-1">
                    <textarea id="template_html" name="template_html">{{ str_replace('<!-- pagebreak -->', '<div class="page-break"></div>', old('template_html', $template->template_html)) }}</textarea>
                </div>
            </div>
                                    ...
                                    <div class="flex items-center gap-4">
                                        <div class="w-8 h-8 rounded-lg bg-indigo-500/10 flex items-center justify-center text-indigo-400 group-hover:scale-110 transition-transform">
                                            <i class="fas {{ 
                                                ...
                                                (Str::contains($group, 'Persetujuan') ? 'fa-check-double' : 'fa-layer-group')))) 
                                            }} text-xs"></i>
                                        </div>
...
<script>
    ...
    function initDocumentDesigner() {
        ...
        const readyIndicator = document.getElementById('editor-ready');
        ...
        // Handle QR Toggle (if exists in edit)
        const qrToggle = document.getElementById('qr_code_enabled');
        const qrConfig = document.getElementById('qr-config');
        if (qrToggle && qrConfig) {
            qrToggle.addEventListener('change', () => {
                qrConfig.classList.toggle('hidden', !qrToggle.checked);
            });
        }
            ...
            tinymce.init({
                ...
                init_instance_callback: function(editor) {
                    ...
                    if (readyIndicator) {
                        ...
                        setTimeout(() => readyIndicator.classList.add('hidden'), 3000);
                    }
                },
                setup: function(editor) {
                    ...
                    // Aggressive cleanup for empty paragraphs after page breaks
                    editor.on('SetContent', function() {
                        const markers = editor.dom.select('div.page-break');
                        markers.forEach(marker => {
                            ...
                            while (next) {
                                ...
                                // If it's an empty or junk paragraph, remove and continue
