...
@section('title', 'Tambah Template Dokumen: ' . $seminarJenis->nama)
...
<div class="max-w-[98%] mx-auto px-2 py-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Tambah Template: {{ $seminarJenis->nama }}</h1>
            <p class="text-slate-500 mt-1 flex items-center gap-2">
                <i class="fas fa-file-signature text-indigo-500"></i> New Seminar Document Designer for <strong>{{ $seminarJenis->nama }}</strong>
            </p>
        </div>
    ...
    <form id="master-form" method="POST" action="{{ route('admin.document-template.store', $seminarJenis) }}" enctype="multipart/form-data">
        @csrf
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-8">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    <div class="form-group lg:col-span-1">
                        ...
                        <input name="nama" value="{{ old('nama') }}" class="w-full px-4 py-2.5 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 transition-all text-sm" placeholder="Contoh: Berita Acara" required>
                    </div>
                    ...
                    <div class="form-group lg:col-span-1">
                        ...
                        <input name="kode" value="{{ old('kode') }}" class="w-full px-4 py-2.5 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 transition-all font-mono text-xs" placeholder="berita_acara_skripsi" required>
                    </div>
                    ...
                    <div class="form-group lg:col-span-1">
                        ...
                        <select name="header_visibility" id="header_visibility" class="w-full px-4 py-2.5 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 transition-all text-sm">
                            <option value="all" {{ old('header_visibility') == 'all' ? 'selected' : '' }}>Semua Halaman</option>
                                ...
                                <div class="flex items-center gap-6">
                                    ...
                                    <label class="flex items-center gap-2 cursor-pointer group">
                                        <input type="checkbox" name="download_rules[disetujui][]" value="dosen" 
                                            {{ in_array('dosen', old('download_rules.disetujui', [])) ? 'checked' : '' }}
                                            class="w-5 h-5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
...
<script>
    ...
    function initDocumentDesigner() {
        ...
        if (saveBtn) {
            // Remove old listeners if any (though AJAX should have replaced the element)
        ...
        const initMCE = () => {
            ...
            // Clean up existing instances to prevent ghosting
