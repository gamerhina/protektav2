                    ...
                    @if($surat->status !== 'ditolak')
                        @if($surat->status === 'selesai' || ($surat->status === 'dikirim' && $surat->jenis?->is_uploaded))
                            @if($surat->jenis?->is_uploaded)
                                 <a href="{{ route('mahasiswa.surat.preview', $surat) }}" target="_blank" class="w-full py-3 rounded-xl bg-emerald-600 text-white font-bold text-xs flex items-center justify-center gap-2 hover:bg-emerald-700 shadow-lg shadow-emerald-100 transition-all">
                            ...
                            @if(!$surat->jenis?->is_uploaded && $templates->isEmpty())
                                <a href="{{ route('mahasiswa.surat.preview-html', $surat) }}" target="_blank" class="w-full py-3 rounded-xl bg-blue-600 text-white font-bold text-xs flex items-center justify-center gap-2 hover:bg-blue-700 shadow-lg shadow-blue-100 transition-all">
